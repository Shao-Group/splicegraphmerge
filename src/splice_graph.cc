...
int splice_graph::build(istream &fin, const string &g, const string &c)
{
	gid = g;
	chrm = c;
	...
	while(fin.getline(line, 10240, '\n'))
	{
		...
		if(string(name) == "node")
		{
			...
			double weight;
			vertex_info vi;
			sstr>>index>>weight>>vi.lpos>>vi.rpos;

			for(int k = num_vertices(); k < index + 1; k++) add_vertex();
...
int splice_graph::write(ostream &fin) const
{
	fin<<fixed;
	fin.precision(2);
	int n = num_vertices();
	
	fin << "# " << gid.c_str() << " " << chrm.c_str() << " " << n << endl;
	for(int i = 0; i < n; i++)
	{
		double weight = get_vertex_weight(i);
		vertex_info vi = get_vertex_info(i);
		fin<<"node " << i << " "<< weight << " " << vi.lpos << " " << vi.rpos << endl;
	}
